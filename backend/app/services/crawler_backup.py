...
class Crawler:
    ...
    async def _crawl_url(self, url: str, depth: int, source_url: Optional[str]) -> Tuple[Optional[Page], Optional[SEOMetadata]]:
        ...
        try:
            ...
            # Return the page and metadata (pages_crawled incremented in _crawl loop after successful save)
            ...
            try:
                ...
                result = supabase_client.table("issues").insert(issue_data).execute()
                if hasattr(result, "error") and result.error:
            ...
            return None, None  # Return None to indicate failure (don't increment pages_crawled)
